...
func TestParseFilterlogRFC5424(t *testing.T) {
	...
	// As OPNsense writes it: the leaf anchor name, action "match" for a
	// match rule, sub-rule number = the rule's index inside the anchor.
	v4 := `<134>1 2026-09-25T17:01:02-05:00 OPNsense.internal filterlog 88380 - [meta sequenceId="2"] 0,7,policy,0,vtnet0,match,match,in,4,0x10,,64,22692,0,DF,17,tcp,60,192.168.2.47,34.249.231.250,51234,443,0,S,1,,65535,,mss;nop;wscale`
	l, ok := parseFilterlog(v4, now)
	if !ok || !isPolicyAnchor(l.Anchor) || l.SubRuleNr != 7 || l.RuleNr != 0 || l.Action != "match" || l.Dir != "in" {
	...
	v6 := `<134>1 2026-09-25T17:01:03-05:00 OPNsense filterlog 61012 - [meta sequenceId="10"] 3,,flowsight/policy,0,igc1,match,match,in,6,0x00,0x00000,64,udp,17,80,2600:1700:3ab0:f43f::19e6,2a05:d018::1,40000,53,72`
	...
	if !isPolicyAnchor("policy") || !isPolicyAnchor("flowsight/policy") || isPolicyAnchor("web") {
		t.Fatal("anchor names")
	}
