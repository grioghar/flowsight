...
func TestPFStateFixture(t *testing.T) {
	// This test verifies the fixture is well-formed
	if len(pfctlSSOutput) == 0 {
		t.Fatal("empty pfctl output")
	}
	...
	for _, expected := range tests {
		if !stringContains(pfctlSSOutput, expected) {
			t.Errorf("pfctl output missing %q", expected)
		}
	}
}

func TestPFLineBasics(t *testing.T) {
	m := &Module{}
	...
	tests := []struct {
	...
	}{
		{
			...
			line:      "tcp 192.168.1.100:54321 -> 10.0.0.1:443 ESTABLISHED:ESTABLISHED",
			wantProto: "tcp",
			wantSrc:   "192.168.1.100:54321",
			wantDst:   "10.0.0.1:443",
		},
		{
			name:      "udp single",
			line:      "udp 192.168.1.103:53422 -> 8.8.8.8:53 SINGLE:SINGLE",
			wantProto: "udp",
			wantSrc:   "192.168.1.103:53422",
			wantDst:   "8.8.8.8:53",
		},
		{
			name:      "icmp",
			line:      "icmp 192.168.1.1 -> 8.8.8.8 SINGLE:SINGLE",
			wantProto: "icmp",
			wantSrc:   "192.168.1.1",
			wantDst:   "8.8.8.8",
		},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			state := m.parsePFLine(tt.line)
			if state == nil {
				t.Fatal("parsePFLine returned nil")
			}
			if state.Proto != tt.wantProto {
				t.Errorf("proto: got %q, want %q", state.Proto, tt.wantProto)
			}
			if state.Src != tt.wantSrc {
				t.Errorf("src: got %q, want %q", state.Src, tt.wantSrc)
			}
			if state.Dst != tt.wantDst {
				t.Errorf("dst: got %q, want %q", state.Dst, tt.wantDst)
			}
		})
	}
}
