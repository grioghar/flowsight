...
FS.registerPage("proxmox", {
  title: 'Proxmox', refresh: 30,
  async render(el) {
    const { esc, get, post } = FS;
    let currentTab = 'inventory';
    let proxmoxMapData = null;
    ...
    const renderPage = async () => {
      el.innerHTML = `
        <div class="panel">
          ...
          <div class="proxmox-tabs">
            <button class="proxmox-tab ${currentTab === 'inventory' ? 'active' : ''}" data-tab="inventory">Inventory</button>
            <button class="proxmox-tab ${currentTab === 'map' ? 'active' : ''}" data-tab="map">Map</button>
          </div>

          <div id="proxmoxInventory" class="proxmox-tab-content ${currentTab === 'inventory' ? 'active' : ''}">
          ...
          <div id="proxmoxMap" class="proxmox-tab-content ${currentTab === 'map' ? 'active' : ''}">
      ...
      // Wire up handlers
      FS.$$('[data-tab]', el).forEach(btn => btn.onclick = async (e) => {
        const tab = btn.dataset.tab;
        currentTab = tab;
        FS.$$('.proxmox-tab', el).forEach(t => t.classList.remove('active'));
        btn.classList.add('active');
        FS.$$('.proxmox-tab-content', el).forEach(c => c.classList.remove('active'));
        const content = FS.$('#proxmox' + tab.charAt(0).toUpperCase() + tab.slice(1), el);
        if (content) content.classList.add('active');
        if (tab === 'map') await loadProxmoxMap();
        else await loadProxmoxInventory();
      });

      FS.$$('[data-act="poll"]', el).forEach(btn => btn.onclick = async () => {
        try {
          const res = await post("/api/proxmox/poll", {});
          FS.toast("Poll started", "info");
          setTimeout(() => { loadProxmoxInventory(); if (currentTab === 'map') loadProxmoxMap(); }, 2000);
        } catch (err) {
      ...
      await loadProxmoxInventory();
    };

    const loadProxmoxInventory = async () => {
      try {
        const inv = await get("/api/proxmox/inventory");
        const status = await get("/api/proxmox/status");
        ...
        if (status.error) {
          html += '<div class="error-msg"><strong>Last poll error:</strong> ' + esc(status.error) + '</div>';
        }
          ...
          html += `
            <div class="node-card">
              ...
              <div class="node-stat">
                ...
                <span>${esc(node.pve_version)}</span>
              </div>
              <div class="node-stat">
                <span>Kernel:</span>
                <span>${esc(node.kernel)}</span>
              </div>
              <div class="node-stat">
                <span>CPU:</span>
        ...
        for (const guest of inv.guests || []) {
          ...
          html += `
            <tr>
              ...
              <td>
                <button class="btn btn-sm" data-notes="${esc(guest.vmid)}|${esc(guest.node)}">Notes</button>
              </td>
            </tr>
          `;
        }
        ...
        FS.$$('[data-notes]', document.getElementById("proxmoxInventoryContent")).forEach(btn => btn.onclick = async () => {
          const [vmid, node] = btn.dataset.notes.split('|');
          const name = btn.parentElement.parentElement.children[1].textContent;
          try {
            const preview = await get("/api/proxmox/notes/preview?vmid=" + vmid + "&node=" + esc(node));
            FS.modal(`<h2>Notes Preview: ${esc(name)}</h2><div style="font-family: monospace; white-space: pre-wrap; background: var(--color-mono-bg); padding: 10px; border-radius: 4px; max-height: 400px; overflow-y: auto;">
${esc(preview.block)}
            </div>
            <div style="margin-top: 20px; display: flex; gap: 10px;">
              <button class="btn" data-write="${esc(vmid)}|${esc(node)}">Write to Proxmox</button>
              <button class="btn btn-secondary" data-close>Close</button>
            </div>`);
            // Wire up write button
            FS.$$('[data-write]', FS.$('#modal-body')).forEach(wb => wb.onclick = async () => {
              const [wvmid, wnode] = wb.dataset.write.split('|');
              if (!confirm("Write notes to guest on Proxmox? This will update the description field.")) return;
              try {
                await post("/api/proxmox/notes/write", { vmid: parseInt(wvmid), node: wnode });
        ...
        document.getElementById("proxmoxInventoryContent").innerHTML = '<div class="error-msg">Error loading Proxmox data: ' + esc(err.message) + '</div>';
      }
    };

    const loadProxmoxMap = async () => {
      try {
        const inv = await get("/api/proxmox/inventory");
        if (!inv.nodes || inv.nodes.length === 0) {
        ...
        let mapHours = 24;
        const mapData = await get(`/api/proxmox/map?hours=${mapHours}`);
        proxmoxMapData = mapData;

        const layout = FS.proxmox.layout(mapData.guests, inv.nodes);
        let html = `
          <div class="map-window-buttons">
            <button class="btn active" data-win="1">1h</button>
            <button class="btn" data-win="24">24h</button>
            <button class="btn" data-win="168">7d</button>
            <button class="btn" style="margin-left:auto;" data-export="map">Export JSON</button>
          </div>
        ...
        for (const guest of (mapData.guests || [])) {
          ...
          html += `
            <rect x="${pos.x}" y="${pos.y}" width="${pos.nodeWidth}" height="${pos.nodeHeight}"
                  ...
                  data-req="${esc(guest.vmid)}|${esc(guest.node)}"
                  style="cursor:pointer;" />
            <text x="${pos.x + 10}" y="${pos.y + 20}" class="guest-box-text" font-weight="bold">${esc(guest.name)}</text>
            <text x="${pos.x + 10}" y="${pos.y + 35}" class="guest-box-text">${guest.vmid}</text>
            <text x="${pos.x + 10}" y="${pos.y + 50}" class="guest-box-text">${esc(ips)}</text>
          `;
        }
        ...
        // Wire up map window buttons
        FS.$$('[data-win]', document.getElementById("proxmoxMapContent")).forEach(btn => btn.onclick = async () => {
          mapHours = parseInt(btn.dataset.win);
          FS.$$('.map-window-buttons button', document.getElementById("proxmoxMapContent")).forEach(b => b.classList.remove('active'));
          btn.classList.add('active');
          await loadProxmoxMap();
        });

        // Wire up export map button
        FS.$$('[data-export="map"]', document.getElementById("proxmoxMapContent")).forEach(btn => btn.onclick = async () => {
          const hours = parseInt(btn.parentElement.querySelector('[data-win].active')?.dataset.win || 24);
          const data = await get(`/api/proxmox/map?hours=${hours}`);
        ...
        // Wire up requirement boxes
        FS.$$('[data-req]', document.getElementById("proxmoxMapContent")).forEach(rect => rect.onclick = async () => {
          const [vmid, node] = rect.dataset.req.split('|');
          if (!proxmoxMapData || !proxmoxMapData.requirements) {
            FS.toast('Requirements not found', 'error');
            return;
          }
          const key = vmid + ':' + node;
          const req = proxmoxMapData.requirements[key];
          if (!req) {
            FS.toast('Requirements not found', 'error');
            return;
          }
          const md = FS.proxmox.requirementsMarkdown(req);
          FS.modal(`<h2>Requirements: VMID ${esc(vmid)}</h2><div style="white-space:pre-wrap;font-family:monospace;font-size:0.9em;max-height:500px;overflow-y:auto;">
${esc(md)}
          </div>
          <div style="margin-top:20px;display:flex;gap:10px;">
            <button class="btn" data-export-req="${esc(vmid)}|${esc(node)}">Export Markdown</button>
            <button class="btn btn-secondary" data-close>Close</button>
          </div>`);
          // Wire up export requirement button
          FS.$$('[data-export-req]', FS.$('#modal-body')).forEach(eb => eb.onclick = async () => {
            const [evmid, enode] = eb.dataset.exportReq.split('|');
            const req = await get(`/api/proxmox/requirements?vmid=${evmid}&node=${encodeURIComponent(enode)}`);
            const md = FS.proxmox.requirementsMarkdown(req);
            const blob = new Blob([md], {type: 'text/markdown'});
            const url = URL.createObjectURL(blob);
            const a = document.createElement('a');
            a.href = url;
            a.download = `proxmox-guest-${evmid}-requirements.md`;
            a.click();
          });
        });
      } catch (err) {
        document.getElementById("proxmoxMapContent").innerHTML = '<div class="error-msg">Error loading map: ' + esc(err.message) + '</div>';
      }
    };
