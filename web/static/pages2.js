...
(function () {
  ...
  FS.policyEditor = function (pol, doc, caps) {
    ...
    FS.modal(`<h2>${isNew ? 'New policy' : 'Edit policy'}</h2><form class="f">
      ...
      <div class="actions"><button class="btn primary">Save</button><button type="button" class="btn" onclick="FS.closeModal()">Cancel</button></div></form>`, (b) => {
  ...
  FS.registerPage('audit', {
    ...
    async render(el) {
      ...
      FS.$$('[data-diff]', el).forEach(b => b.onclick = async () => { const r = await get('/api/system/changes?id=' + b.dataset.diff); FS.modal(`<h2>${esc((r.change || {}).subject || 'change')}</h2><pre class="code">${FS.diffHtml((r.change || {}).diff || '')}</pre><div class="actions"><button class="btn" onclick="FS.closeModal()">Close</button></div>`); });
    }
  });
})();
