...
(function () {
  ...
  FS.registerPage('enroll', {
    ...
    async render(el, ctx) {
      ...
      if (s.error) { el.innerHTML = FS.err(s.error); return; }
      const zones = (z.zones || (z.document || {}).zones || []);
      const devices = dv.devices || [];
      const mode = s.mode || (z.mode) || 'monitor';
      ...
      <div class="grid cols-2" style="margin-top:14px">${card('Zones (zones.json)', `<form class="f" id="zf"><textarea name="doc" style="min-height:280px">${esc(JSON.stringify(z.document || z, null, 1))}</textarea><div class="actions"><button class="btn primary">Save zones</button></div></form>`)}${card('Classification rules (enroll-rules.json)', `<form class="f" id="rf"><textarea name="doc" style="min-height:280px">${esc(JSON.stringify(ru.document || ru, null, 1))}</textarea><div class="actions"><button class="btn primary">Save rules</button></div></form>`)}</div>`;
