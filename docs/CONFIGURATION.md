...
| `osm_telecom` | Use OpenStreetMap telecom lines (low weight) | bool | `true` | Overpass-fetched telecom/communication lines, one region per six-hour run, kept a month, counted at half weight in the expected time only. |
