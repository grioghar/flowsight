...
| GET | `/api/paths/destinations` | Destinations with a measured route | limit (rows) `bytes_in`/`bytes_out` are from the five-minute rollups, so lag the newest flow by up to five minutes. |
